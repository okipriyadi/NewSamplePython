...
def receive(channel):
    ...
    print "data received [before ntohl]= ", size
    try :
        size = socket.ntohl(struct.unpack("L", size)[0])
        print "data received [after ntohl]= ", size
    except struct.error, e:
        return ''
    buf =''
    while len(buf) < size:
        buf = channel.recv(size - len(buf))
    hasil = cPickle.loads(buf)[0]
    print "cPikle.load = ", hasil
    return  hasil

"""
The send() method takes one named argument channel and positional argument *args . It serializes the data 
using the dumps() method from the cPickle module. It determines the size of the data using the 
struct module. Similarly, receive() takes one named argument channel .
