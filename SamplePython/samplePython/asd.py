...
from peewee import PostgresqlDatabase, Model, CharField, IntegerField, DateTimeField, TextField, ForeignKeyField, \
    FloatField, BigIntegerField, Check
...
# Peewee init
########################################################################################################################
getLogger('peewee').disabled = True
db_connection = PostgresqlDatabase("igg_db", user="igg", password="igg", host='10.0.1.55')
...
class Aircraft(Model):
    ...
    sbd_mode = IntegerField(default=0,
                            choices=((1, 'always disabled'),
                                     (2, 'enabled but subject to quota'),
                                     (3, 'always enabled')))
...
class SBDEventCache(BaseModel):
    ...
    event = CharField(choices=((0, 'message sent'),
                                 (1, 'message sent and replied'),
                                 (3, 'failed'),))
...
class SOREventCache(Model):
    ...
    event = CharField(choices=((1, 'message sent'),
                               (2, 'message sent and replied'),
                               (3, 'mesage failed')))
    fetched = BooleanField(default=False)
